...
export type ClientLogsResolverTestContext = {
  ...
  clientLoggerService: ReturnType<typeof createClientLoggerServiceMock>;
};

export async function createClientLogsResolverContext(): Promise<ClientLogsResolverTestContext> {
  const clientLoggerService = createClientLoggerServiceMock();

  const resolver = await createContext(ClientLogsResolver, [
    {
